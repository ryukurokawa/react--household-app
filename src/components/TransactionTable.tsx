...
// interface TransactionTableProp {
...
export default function TransactionTable(
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      let newSelected:string[] = monthlyTransactions.map((n) => n.id);
  ...
  const visibleRows = React.useMemo(
    () =>{
      const sortedMonthlyTransactions = [...monthlyTransactions].sort((a,b) => compareDesc(parseISO(a.date), parseISO(b.date)))
  ...
  interface FinancialItemProp {
    title:string, 
    value:number, 
    color:string
  }
 function FinancialItem({title, value , color}:FinancialItemProp){
    return(  
    <Grid item xs={4} textAlign={"center"} sx={{borderBottom:"1px solid rgba(224,224,224,1)"}}>
      <Typography variant='subtitle1' component={"div"}>{title}</Typography>
      <Typography component={"span"} fontWeight={"fontWeightBold"} sx={{color:color, fontSize:{xs:".8rem", sm:"1rem", md:"1.2rem"}, wordBreak:"break-word"}}>￥{formatCurrency(value)}</Typography>
      </Grid>
    )
  }
              ...
              {visibleRows.map((transaction, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="left" sx={{display:"flex", alignItems:"center"}}>{IconComponents[transaction.category]}{transaction.category}</TableCell>
